...
MockExamRenderer.prototype.mockUrl = function() {
  ...
  return '/alevelrevise/mock-exams/' + this.meta.subject +
    tierPart + '.json';
};
...
MockExamRenderer.prototype.submit = function() {
  ...
  try {
    ...
    var mk = (this.meta.subject + '|' + (this.meta.tier || '')).toLowerCase();
...
function initMockExams() {
  ...
  if (!container) return;
  var m = window.location.pathname.match(/([a-z0-9-]+)\.html$/);
  if (!m || m[1] === 'index' || m[1] === 'privacy') return;
  var subject = m[1];
  var boards = [];
